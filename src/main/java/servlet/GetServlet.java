...
public class GetServlet extends HttpServlet {
    ...
    private void getPlantInfo(HttpServletResponse resp, String plant_id) throws ServletException, IOException {
		...
		try {
			...
			String plantInfo = "{";
			if(rs != null && rs.next()){
				plantInfo += "\"Plant ID\":\"" + rs.getString("plant_id") + "\", \"Species\":\"" + rs.getString("species") + "\", \"Variety\":\"" + rs.getString("variety") + "\", \"Visual Tag\":\"" + rs.getString("visual_tag") + "\", \"Date Registered\":\"" + rs.getDate("date") + "\", \"Longitude\":\"" + rs.getDouble("longitude") + "\", \"Latitude\":\"" + rs.getDouble("latitude") + "\", \"Notes\":\"" + rs.getString("notes") + "\"";
				plantInfo += "}";
    ...
    private void getActivityInfo(HttpServletResponse resp, int activity_id) throws ServletException, IOException {
		...
		ResultSet rs = null;
		try {
			ps = con.prepareStatement("select h.variety, g.species, h.plant_id, h.date, h.time, h.type_id, h.notes, h.activity_type from species g inner join (select e.variety as variety, e.species_id as species_id, f.plant_id as plant_id, f.date as date, f.time as time, f.type_id as type_id, f.notes as notes, f.activity_type as activity_type from variety e inner join (select c.variety_id as variety_id, d.plant_id as plant_id, d.date as date, d.time as time, d.type_id as type_id, d.notes as notes, d.activity_type as activity_type from plant_record c inner join (select a.plant_id as plant_id, a.date as date, a.time as time, a.type_id as type_id, a.notes as notes, b.activity_type as activity_type from activities a inner join activity_types b on a.type_id=b.type_id where a.activity_id=? order by a.date DESC, a.time DESC LIMIT 10) d on c.plant_id=d.plant_id) f on e.variety_id=f.variety_id) h on g.species_id=h.species_id");
